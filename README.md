...
### Why This Story
I chose the WSJ inflation piece because it makes a strong but narrow claim — that February 2026 inflation looks calm yet fragile because of oil — and I thought a remix could test whether the risk picture is really that simple. The article uses a single monthly snapshot, which felt like a natural starting point for adding temporal context and alternative explanations through visualization.
...
**Chart type choices:** I deliberately picked three different chart forms (line, diverging bar, scatter) so that each view forces a different mode of reading — trend perception, ranking comparison, and correlation assessment. The fourth chart (counter-argument) reuses a line form because the comparison it makes (energy vs services over time) requires a shared temporal axis for the reader to see the divergence pattern clearly.

**Color palette:** I kept a small, intentional palette — red for headline/energy (the "hot" risk channel), blue for core/negative values, and green for services — so that each color carries a consistent meaning across all four charts. I avoided Tableau-style rainbow palettes to keep the story focused.

**Interaction model:** Every chart supports hover tooltips for casual exploration and click-to-select for deep inspection. When you click a month in any chart, all four update to show that month's data. This coordinated selection was a deliberate choice to let readers test the article's claim on any month they want, not just the one the WSJ highlighted.

**Sort toggle on the bar chart:** I added alphabetical and value-based sorting because the two orderings answer different questions — "which categories are hottest?" vs "where is a specific category I care about?" This small control significantly increases reader agency.

**Visual restraint:** I intentionally kept the page simple — no hero images, no animated transitions, no gradient backgrounds. The typography is utilitarian and the cards are flat-bordered. This was a conscious decision: the story should earn attention through the data and interaction, not through visual spectacle.
...
1. **Live Deployment (1pt):** Published via GitHub Pages — link at the top of this document and in the story itself.
...
3. **New Technique (3pts):** Scrollytelling walkthrough with IntersectionObserver-driven checkpoints — see addendum below.
4. **Multiple Views / Counter-Argument (3pts):** Section 4 explicitly argues that sticky services inflation is as important as oil risk, with side-by-side claim framing and a dedicated comparison chart — see addendum below.

---

## Addendum: Effect of New Technique (Scrollytelling)

I changed the general presentation approach from a static magazine-style layout — where visualizations appear inline like paper figures — to a **scrollytelling** model. In the scrollytelling section at the top of the page, four checkpoint cards sit on the left while a sticky summary panel stays visible on the right. As the reader scrolls (or clicks) through each checkpoint, the page updates the selected date, highlights the most relevant chart section, and refreshes the stat chips. An IntersectionObserver triggers the transitions automatically.
...
- **Coordinated narrative pacing:** Because each scrollytelling step also drives the view coordination system, the reader sees the same month reflected simultaneously across different chart types. This makes cross-chart comparisons effortless — you can see, for example, that June 2022 has high oil *and* broad category pressure at the same time, reinforcing the causal link.
...
The original WSJ article emphasizes oil as the primary near-term inflation risk. This remix presents an explicit counter-argument: **persistent services inflation can keep pressure elevated even when oil prices retreat.**
...
- **The category bar chart**, which (when set to February 2026) shows that several service-heavy categories — shelter, medical care, transportation services — remain above the overall CPI line even as energy has cooled.
...
## AI Use Disclosure
OpenAI Codex (GPT-5) was used for implementation support, D3 debugging, and copy editing. Story framing, data selection, and design decisions were authored by Clay Parr.
